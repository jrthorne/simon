 ...
 * parameter is not specified, it creates the stop event, which stops the
 ...
 * @returns Array of events, with a maximum of two events (start and stop)
 */
...
MidiEvent.createNote = function(note, sustained) {
    ...
    var events = [];
    events.push(MidiEvent.start(note));

    // If there is a |sustained| parameter, the note will keep playing until
    // a stop event is issued for it.
    if (!sustained) {
        // The stop event will be the one that is passed the actual duration
        ...
        events.push(MidiEvent.stop(note, note.duration || DEFAULT_DURATION));
    }
 ...
 */
MidiEvent.start = function(note, duration) {
...
MidiEvent.stop = function(note, duration) {
    return new MidiEvent({
        time:    note.duration || duration || 0,
        type:    EVT_NOTE_OFF,
        channel: note.channel || DEFAULT_CHANNEL,
