	...
	for (var n = 0; n < length && messages < 100; n++) {
		...
		switch (event.subtype) {
			case 'start':
				...
				eventQueue.push({
					event: event,
					source: MIDI.start(channel, event.noteNumber, event.velocity, currentTime / 1000 + ctx.currentTime),
			...
			case 'stop':
				...
				eventQueue.push({
					event: event,
					source: MIDI.stop(channel, event.noteNumber, currentTime / 1000 + ctx.currentTime),
