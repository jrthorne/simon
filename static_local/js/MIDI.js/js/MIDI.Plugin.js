...
if (window.AudioContext || window.AudioContext) (function () {
	...
	root.start = function (channel, note, velocity, delay) {
...
//		source.playbackRate.value = 2;
	...
	root.chordOn = function (channel, chord, velocity, delay) {
		...
		for (var n = 0, length = chord.length; n < length; n ++) {
			ret[note = chord[n]] = root.start(channel, note, velocity, delay);
		}
		return ret;
	};

	// FIX: needs some way to fade out smoothly..
	root.stop = function (channel, note, delay) {
...
(function () {
	...
	root.confirm = function (plugin) {
		...
		MIDI.start = function (channel, note, velocity, delay) {
			if (delay) {
				var interval = window.setTimeout(function() { 
		...
		MIDI.stop = function (channel, note, delay) {
			if (delay) {
				var interval = window.setTimeout(function() { 
